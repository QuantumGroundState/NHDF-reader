...
def read_data_and_metadata(path: pathlib.Path) -> DataAndMetadata.DataAndMetadata:
    with h5py.File(str(path), "r") as f:
        ...
        return DataAndMetadata.DataAndMetadata(
            lambda: data, data_shape_and_dtype=dm.data_shape_and_dtype,
...
class NHDFApp:
    ...
    def _build_ui(self):
        ...
        ir.columnconfigure((0, 1, 2, 3), weight=1)
        def mkb(p, l, c):
        ...
        self.lb_fw = mkb(ir, "FWHM", 0)
        self.lb_bs = mkb(ir, "Bin(spectrum)", 1)
        self.lb_bx = mkb(ir, "Bin(x/dim0)", 2)
        self.lb_by = mkb(ir, "Bin(y/dim1)", 3)
    ...
    def _setup_esl(self):
        ...
        zi = _e0idx(self.current_data); self.sl_e.set(zi)
    ...
    def _draw_img(self, ei):
        ...
        self.ai.set_title(f"E={ev:.4g} {ue} [slice {ei}]", fontsize=_FP)
        ...
        if self._roi is not None:
            ix0, iy0, ix1, iy1 = self._roi
            rl = oy + sy * iy0
            rt = ox + sx * ix0
            rw = sy * (iy1 - iy0 + 1)
            rh = sx * (ix1 - ix0 + 1)
            rect = Rectangle((rl, rt), rw, rh, linewidth=1.5,
                              edgecolor="lime", facecolor="lime", alpha=0.2)
            self.ai.add_patch(rect)
        if self._roi2 is not None:
            ix0, iy0, ix1, iy1 = self._roi2
            rl = oy + sy * iy0
            rt = ox + sx * ix0
            rw = sy * (iy1 - iy0 + 1)
            rh = sx * (ix1 - ix0 + 1)
            rect2 = Rectangle((rl, rt), rw, rh, linewidth=1.5,
                               edgecolor="cyan", facecolor="cyan", alpha=0.2)
            self.ai.add_patch(rect2)
    ...
    def _on_img_motion(self, ev):
        ...
        self._roi = (min(ix0, ix1), min(iy0, iy1), max(ix0, ix1), max(iy0, iy1))
        r = self._roi
        self.lb_roi.config(text=f"ROI: x[{r[0]}:{r[2]+1}] y[{r[1]}:{r[3]+1}]")
        self._draw_img(self.sl_e.get())
    ...
    def _roi_spectrum(self):
        ...
        ix0, iy0, ix1, iy1 = self._roi
        sp = d[ix0:ix1+1, iy0:iy1+1, :].sum(axis=(0, 1))
    ...
    def _place_roi2(self):
        ...
        nx, ny = self.current_data.data.shape[0], self.current_data.data.shape[1]
        off = max(3, w // 2)
        nx0 = min(ix0 + off, nx - 1 - w); ny0 = min(iy0 + off, ny - 1 - h)
        nx0 = max(0, nx0); ny0 = max(0, ny0)
        self._roi2 = (nx0, ny0, nx0 + w, ny0 + h)
        r = self._roi2
        self.lb_roi2.config(text=f"Bkg: x[{r[0]}:{r[2]+1}] y[{r[1]}:{r[3]+1}]")
        self._draw_img(self.sl_e.get()); self._draw_pt()
    ...
    def _move_roi2(self, dx, dy):
        ...
        ax0, ay0, ax1, ay1 = self._roi2_anchor
        w = ax1 - ax0; h = ay1 - ay0
        nx, ny = self.current_data.data.shape[0], self.current_data.data.shape[1]
        nx0 = max(0, min(nx - 1 - w, ax0 + dx))
        ny0 = max(0, min(ny - 1 - h, ay0 + dy))
    ...
    def _draw_pt(self):
        ...
        fw_roi = _fwhm(xc, roi_sp.astype(float)) if roi_sp is not None else None
        fw_roi2 = _fwhm(xc, roi2_sp.astype(float)) if roi2_sp is not None else None

        # Y limits
        ym = max(float(numpy.max(sp_cur)), 1e-30)
        if roi_sp is not None:
            ym = max(ym, float(numpy.max(roi_sp)))
        if roi2_sp is not None:
            ym = max(ym, float(numpy.max(roi2_sp)))
        ...
        # Build FWHM annotation parts
        fwhm_parts = [f"FWHM cur: {fw_cur:.4g} {u}"]
        if fw_roi is not None:
            fwhm_parts.append(f"FWHM ROI: {fw_roi:.4g} {u}")
        if fw_roi2 is not None:
            fwhm_parts.append(f"FWHM Bkg: {fw_roi2:.4g} {u}")

        # Plot cursor
        self.ap.plot(xc, sp_cur, lw=0.6, color="tab:orange",
                     label=f"cursor (FWHM {fw_cur:.4g})")

        # Plot ROI
        if roi_sp is not None:
            self.ap.plot(xc, roi_sp, lw=0.6, color="lime",
                         label=f"ROI sum (FWHM {fw_roi:.4g})")

        # Plot ROI2
        if roi2_sp is not None:
            self.ap.plot(xc, roi2_sp, lw=0.6, color="cyan",
                         label=f"Bkg sum (FWHM {fw_roi2:.4g})")
        ...
        # Text annotation with FWHM values in the plot
        self.ap.text(0.01, 0.97, " | ".join(fwhm_parts),
   ...
   # ---- file actions ----
    ...
    def _save_file(self):
        ...
        dn = self.current_path.stem+cs+"_copy.nhdf" if self.current_path else "out.nhdf"
    ...
    def _export_csv(self):
        if not self.current_data: messagebox.showwarning("", "Open first."); return
        d = self.current_data.data; cs = self._comment_suffix()
        if d.ndim <= 2:
            p = filedialog.asksaveasfilename(title="CSV", defaultextension=".csv",
                initialfile=(self.current_path.stem+cs+".csv" if self.current_path else "o.csv"),
                filetypes=[("CSV", "*.csv")])
            if not p: return
            try:
            ...
            if not fld: return
            try:
                b = (self.current_path.stem if self.current_path else "data") + cs
    ...
    def _export_sum(self):
        ...
        p = filedialog.asksaveasfilename(title="Sum spectrum", defaultextension=".csv",
            initialfile=(self.current_path.stem+cs+"_sum.csv" if self.current_path else "sum.csv"),
            filetypes=[("CSV", "*.csv")])
        if not p: return
        try:
    ...
    def _export_cube(self):
        ...
        cs = self._comment_suffix()
        default_name = f"{base}{cs}_x{nx}_y{ny}_z{nz}_{dt}.raw"
    ...
    def _export_roi(self):
        ...
        base = self.current_path.stem if self.current_path else "data"
        cs = self._comment_suffix()
        default_name = f"{base}{cs}_roi_x{ix0}-{ix1}_y{iy0}-{iy1}.csv"
        ...
        try:
            ...
            messagebox.showinfo("Exported",
                f"ROI x[{ix0}:{ix1+1}] y[{iy0}:{iy1+1}]\n"
                f"Pixels summed: {(ix1-ix0+1)*(iy1-iy0+1)}\n"
                f"Channels: {sp.size}\n→ {p}")
        except Exception as e: 
            traceback.print_exc()
            messagebox.showerror("Error", str(e))
    ...
    def _export_bkg(self):
        ...
        base = self.current_path.stem if self.current_path else "data"
        cs = self._comment_suffix()
        default_name = f"{base}{cs}_bkg_x{ix0}-{ix1}_y{iy0}-{iy1}.csv"
    ...
    def _export_sub(self):
        ...
        roi_r = self._roi_spectrum()
        roi2_r = self._roi2_spectrum()
        if roi_r is None or roi2_r is None:
            messagebox.showwarning("Need both", "Draw ROI and place Bkg first.")
            return
        xc, u, sp_roi = roi_r
        _, _, sp_bkg = roi2_r
        diff = sp_roi.astype(float) - sp_bkg.astype(float)
        base = self.current_path.stem if self.current_path else "data"
        cs = self._comment_suffix()
        default_name = f"{base}{cs}_ROIsub.csv"
        p = filedialog.asksaveasfilename(title="Export ROI\u2212Bkg", defaultextension=".csv",
            initialfile=default_name, filetypes=[("CSV", "*.csv")])
        if not p: return
        try:
            h = f"{'energy('+u+')' if u else 'channel'},ROI_minus_Bkg"
            numpy.savetxt(p, numpy.column_stack((xc, diff)), delimiter=",", fmt="%.8g",
                          header=h, comments="")
            self.status.config(text=f"Sub: {pathlib.Path(p).name}")
            messagebox.showinfo("Exported", f"ROI\u2212Bkg subtracted spectrum\n→ {p}")
        except Exception as e: 
            traceback.print_exc()
            messagebox.showerror("Error", str(e))
