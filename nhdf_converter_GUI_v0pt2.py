...
class DM3Writer:
    ...
    def build(self, data_3d, cal_x, cal_y, cal_e, props=None, title='Spectrum Image'):
        """
        ...
        # ImageTags  (3 sub-groups: GMS Version, Meta Data, Nion)
        self._te_hdr(20, 'ImageTags')
        ...
        # Meta Data — tells GMS this is an EELS Spectrum Image
        ...
        # Nion — nhdf properties flattened as string tags
...
def write_raw(out_path, data_3d, cal_x=None, cal_y=None, cal_e=None, cal_i=None):
    """
    ...
    with open(txt_path, 'w') as fh:
        ...
        for label, dim, cal in _cals:
            if cal is not None:
                _off_px = cal[0] / cal[1] if cal[1] != 0.0 else 0.0
                fh.write(f'{label:<14}{dim:<6}{_off_px:<26.10g}{cal[1]:<26.10g}{cal[2]}\n')
            else:
...
def patch_dm3_full(ref_dm3_path, data_3d, cal_x, cal_y, cal_e,
    ...
    for di, cal in ((0, cal_x), (1, cal_y), (2, cal_e)):
        ...
        origin = (-cal[0] / cal[1]) if (di == 2 and cal[1] != 0) else 0.0
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title('NHDF Spectrum Image Converter  v0.2')
    ...
    def _build_ui(self):
        ...
        # Input
        fi = ttk.LabelFrame(self, text='Input File  (.nhdf)', padding=P)
        fi.grid(row=0, column=0, sticky='ew', padx=P, pady=(P, 4))
        self._in_var = tk.StringVar()
        ttk.Entry(fi, textvariable=self._in_var, width=58).grid(row=0, column=0, padx=(0, 6))
        ttk.Button(fi, text='Browse…', command=self._browse_in).grid(row=0, column=1)

        # Format selector
        ff = ttk.LabelFrame(self, text='Output Format  (click to select)', padding=P)
        ff.grid(row=1, column=0, sticky='ew', padx=P, pady=4)
        ...
        # Output
        fo = ttk.LabelFrame(self, text='Output File', padding=P)
        fo.grid(row=2, column=0, sticky='ew', padx=P, pady=4)
        self._out_var = tk.StringVar()
        ttk.Entry(fo, textvariable=self._out_var, width=58).grid(row=0, column=0, padx=(0, 6))
        ttk.Button(fo, text='Save As…', command=self._browse_out).grid(row=0, column=1)

        # Convert button + progress
        self._btn_conv = ttk.Button(self, text='  Convert  ', command=self._convert)
        self._btn_conv.grid(row=3, column=0, pady=8)

        self._prog = ttk.Progressbar(self, mode='indeterminate', length=560)
        self._prog.grid(row=4, column=0, padx=P, pady=2)

        # Log
        fl = ttk.LabelFrame(self, text='Status / Log', padding=4)
        fl.grid(row=5, column=0, sticky='nsew', padx=P, pady=(4, P))
    ...
    def _sel(self, fmt):
        ...
        inp = self._in_var.get()
        if inp:
            self._out_var.set(os.path.splitext(inp)[0] + '.' + fmt)
    ...
    def _browse_in(self):
        ...
        if p:
            self._in_var.set(p)
            self._out_var.set(os.path.splitext(p)[0] + '.' + self._fmt.get())
    ...
    def _convert(self):
        ...
        def _run():
            try:
                ...
                self._log_msg(f'  E range  : {cal_e[0]:.4g} → '
                              f'{cal_e[0] + cal_e[1]*(ne-1):.4g} {cal_e[2]}')
                ...
                if fmt == 'dm3':
                    # ── DM3 ───────────────────────────────────────────────────
                    # Build DM3 from the embedded GMS-verified structural
                    # template; no external reference file required.
                    ...
                    out = build_dm3_from_nhdf(data,
                                              cal_x=cal_x, cal_y=cal_y,
                                              cal_e=cal_e, cal_i=cal_i_dm3,
                                              title=title)
                    ...
                    with open(out_path, 'wb') as fh:
                        fh.write(out)
                        fh.flush()
                        os.fsync(fh.fileno())   # guarantee flush to disk

                    # ── verify the written file is complete ───────────────────
                    self._log_msg('\nVerifying DM3 …')
                    written_size = os.path.getsize(out_path)
                    if written_size != len(out):
                        self._log_msg(
                            f'  ✗ SIZE MISMATCH: wrote {written_size:,} B'
                            f' but expected {len(out):,} B  '
                            f'(disk full or I/O error — file is corrupt!)')
                    else:
                        self._log_msg(f'  File size   : {written_size:,} bytes ✓')

                    # read back and locate SI data
                    ...
                    if off >= 0:
                        self._log_msg(f'  SI data found at byte offset {off:,}  ✓')
                        ...
                        self._log_msg('  First 8 float32 match  ✓' if ok
                                      else '  WARNING: first 8 values differ!')
                    else:
                    ...
                    cal_i = (float(_ic.get('offset', 0.0)),
                             float(_ic.get('scale',  1.0)),
                             str(_ic.get('units', '')))
                    nbytes, txt_path = write_raw(
                        out_path, data,
                        cal_x=cal_x, cal_y=cal_y, cal_e=cal_e, cal_i=cal_i)
                    self._log_msg(f'  Written  : {nbytes:,} bytes'
                                  f'  ({nbytes/1024/1024:.3f} MB)')
                    ...
                    # round-trip check: read back and compare
