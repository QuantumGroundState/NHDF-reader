...
class NHDFApp:
    ...
    def _on_img_motion(self, ev):
        ...
        self._roi = numpy.array([[lo_x, lo_y], [lo_x, hi_y], [hi_x, hi_y], [hi_x, lo_y]])
        self.lb_roi.config(text=self._roi_label_text(self._roi, "ROI"))
        self._draw_img(self.sl_e.get())
   ...
   # ---- file actions ----
    ...
    def _export_roi(self):
        ...
        base = self.current_path.stem if self.current_path else "data"
        cs = self._comment_suffix()
        default_name = f"{base}{cs}_spectrum_roi_x{xs.min()}-{xs.max()}_y{ys.min()}-{ys.max()}.csv"
    ...
    def _export_bkg(self):
        ...
        base = self.current_path.stem if self.current_path else "data"
        cs = self._comment_suffix()
        default_name = f"{base}{cs}_background_bkg_x{xs.min()}-{xs.max()}_y{ys.min()}-{ys.max()}.csv"
    ...
    def _export_sub(self):
        ...
        base = self.current_path.stem if self.current_path else "data"
        cs = self._comment_suffix()
        default_name = f"{base}{cs}_sub_{lbl_sub}{norm_tag}.csv"
